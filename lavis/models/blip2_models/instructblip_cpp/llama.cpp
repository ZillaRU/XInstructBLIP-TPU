...
public:
  ...
  void deinit();
  int forward_first(std::vector<int> &tokens, std::vector<float> &emb0);
  int forward_next();
  std::vector<int> generate(std::vector<int> &history_tokens, int EOS, std::vector<float> &emb0);
  union Fp32{
    uint32_t u;
    float f;
  };
...
uint16_t Llama2::float2uint16(float value)
{
...
void Llama2::init(const std::vector<int> &devices, std::string model_path) {
  ...
  EMB_SIZE = net_embed->stages[0].input_shapes[0].dims[2];
...
int Llama2::forward_first(std::vector<int> &tokens, std::vector<float> &emb0) {
  std::vector<int> position_id(SEQLEN, 0);
  std::vector<uint16_t> attention_mask(SEQLEN * SEQLEN, ATTENTION_MASK);
  std::copy(tokens.begin(), tokens.end(), visited_tokens.data());
  
  token_length = tokens.size() + int(emb0.size()/EMB_SIZE);
  ...
  // emb0 f32 -> f16
  std::vector<uint16_t> emb0_u16(emb0.size());
  for(int i = 0; i < int(emb0.size()); i++) {
    emb0_u16[i] = float2uint16(emb0[i]);
  }
  // d2d out_mem
  bm_memcpy_d2d_byte(bm_handle, out_mem, emb0_u16.size() * sizeof(uint16_t), out_mem, 0, visited_tokens.size() * EMB_SIZE * sizeof(uint16_t));
  // s2d emb0
  bm_memcpy_s2d(bm_handle, out_mem, (void *)emb0_u16.data());
...
std::vector<int> Llama2::generate(std::vector<int> &history_tokens, int EOS, std::vector<float> &emb0) {
  ...
  if ((int)(history_tokens.size() + emb0.size()/EMB_SIZE) > SEQLEN - 10) {
    history_tokens.clear();
  ...
  int token = forward_first(history_tokens, emb0);
...
PYBIND11_MODULE(llama, m) {
    pybind11::class_<Llama2>(m, "llama")
        .def(pybind11::init<>())
        .def("init", &Llama2::init)
        .def("forward_first", &Llama2::forward_first)
        .def("forward_next", &Llama2::forward_next)
        .def("generate", &Llama2::generate)
        .def("deinit", &Llama2::deinit)
        .def_readwrite("SEQLEN", &Llama2::SEQLEN) // read SEQLEN in pipeline.py
        .def_readwrite("token_length", &Llama2::token_length)
        .def_readwrite("temperature", &Llama2::temperature)
        .def_readwrite("top_p", &Llama2::top_p)
        .def_readwrite("repeat_penalty", &Llama2::repeat_penalty)
        .def_readwrite("repeat_last_n", &Llama2::repeat_last_n)
        .def_readwrite("max_new_tokens", &Llama2::max_new_tokens)
        .def_readwrite("generation_mode", &Llama2::generation_mode)
        .def_readwrite("prompt_mode", &Llama2::prompt_mode);
}